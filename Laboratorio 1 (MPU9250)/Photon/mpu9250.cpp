...
#if (PLATFORM_ID == 6)
  STM32_Pin_Info* PIN_MAP3 = HAL_Pin_Map(); // Pointer required for highest access speed
  #define pinLO(_pin) (PIN_MAP3[_pin].gpio_peripheral->BSRRH = PIN_MAP3[_pin].gpio_pin)
  #define pinHI(_pin) (PIN_MAP3[_pin].gpio_peripheral->BSRRL = PIN_MAP3[_pin].gpio_pin)
#else
...
MPU9250::MPU9250(int csPin)
{
   _pinCS=csPin;
   
   for(int i=0;i<4;i++)pinMode(pinsCS[i],OUTPUT);
   for(int i=0;i<4;i++)pinSet(pinsCS[i],HIGH);

}

//--------- Destroyer for Object Class ------------- 
MPU9250::~MPU9250() {
    
}

//--------- This function is used to select Node Sensor ------------- 
void MPU9250::Select(int csPin){
    Desactive();
    _pinCS=csPin;
    Active();
}
...
// --------- This function is used to activate Node sensor ------------- 
void MPU9250::Active(){
    pinSet(pinsCS[_pinCS],LOW); // Select Pin ACTIVATE
...
// --------- This function is used to deactivate Sensor Node -------------
void MPU9250::Desactive(){
    WriteRegister(MPU9250_DIR,INT_PIN_CFG,0x30);//MAG DESCONECTADO (BYPASS_EN bit is configure as 1 // the host application processor will not be able to directly access the auxiliary I2C bus of the MPU-9250 )
    pinSet(pinsCS[_pinCS],HIGH); // Select Pin DESACTIVATE
}
